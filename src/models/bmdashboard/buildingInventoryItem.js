...
const smallItemBase = mongoose.model(
  'smallItemBase',
  smallItemBaseSchema,
  'buildingInventoryItems',
);
...
const largeItemBaseSchema = mongoose.Schema({
  ...
  purchaseStatus: {
    type: String,
    enum: ['Rental', 'Purchase', 'Needed', 'Purchased', 'Rented'],
    default: 'Rental',
  },
  condition: {
    type: String,
  ...
  imageUrl: {
    type: String,
    default: 'https://ik.imagekit.io/tuc2020/wp-content/uploads/2021/01/HW2927.jpg',
  },
  ...
  logRecord: [
    {
      // track tool daily check in/out and responsible user
      date: { type: Date, default: Date.now() },
      createdBy: { type: mongoose.SchemaTypes.ObjectId, ref: 'userProfile' },
...
const largeItemBase = mongoose.model(
  'largeItemBase',
  largeItemBaseSchema,
  'buildingInventoryItems',
);
...
const buildingMaterial = smallItemBase.discriminator(
  'material_item',
  new mongoose.Schema({
...
const buildingConsumable = smallItemBase.discriminator(
  'consumable_item',
  new mongoose.Schema({
    stockUsed: { type: Number, default: 0 }, // stock that has been used up and cannot be reused
    stockWasted: { type: Number, default: 0 }, // ruined or destroyed stock
...
const buildingReusable = smallItemBase.discriminator(
  'reusable_item',
  new mongoose.Schema({
...
const buildingTool = largeItemBase.discriminator(
  'tool_item',
  new mongoose.Schema({
...
const buildingEquipment = largeItemBase.discriminator(
  'equipment_item',
  new mongoose.Schema({
