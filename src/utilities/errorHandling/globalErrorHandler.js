...
function globalErrorHandler(err, req, res, next) {
  ...
  if (!err) {
    transactionName =
      'Critical: err parameter is missing. This is probably due to an improper error handling in the code.';
  }
  // Log the error to Sentry if not in local environment
  if (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'production') {
    Logger.logException(err, transactionName, requestData, trackingId);
  } else {
    console.log(
      `An error occurred. Transaction: ${transactionName} \nRequest Data: ${requestData}`,
    );
    console.error(err);
  }
