...
const routes = function () {
  ...
  noShowRouter.route('/location').get(controller.getNoShowsByLocation);

  // Route for getting no shows by age group
  noShowRouter.route('/age-group').get(controller.getNoShowsByAgeGroup);

  // Route for getting no shows by gender (example)
  noShowRouter.route('/proportions').get(controller.getNoShowProportions);

  noShowRouter.route('/unique-eventTypes').get(controller.getUniqueEventTypes);

  // Route for getting attendance by day
  noShowRouter.route('/by-day').get(controller.getAttendanceByDay);
