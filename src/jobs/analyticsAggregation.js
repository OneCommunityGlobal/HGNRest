...
const aggregateDailyData = async (date) => {
  try {
    ...
    const conversionRate = uniqueSessionsCount > 0 
      ? Math.round((totalApplications / uniqueSessionsCount) * 100) 
      : 0;
    ...
    const metrics = {
      ...
      topJobViews: getTopItems(
        ...
        return {
          ...
          conversionRate: item.count > 0 ? Math.round((applications.totalApplications / item.count) * 100) : 0
        };
      })
    };
...
const generateDailySummary = async (date) => {
  try {
    ...
    if (existingSummary) {
      console.log(`Summary already exists for ${targetDate.toDateString()}, updating...`);
    ...
    console.log(`Daily summary created for ${targetDate.toDateString()}`);
  ...
  console.log('📅 Analytics aggregation job scheduled (every minute for testing)');
};

// Manual function to backfill historical data
const backfillSummaries = async (startDate, endDate) => {
  console.log(`🔄 Backfilling summaries from ${startDate} to ${endDate}`);
  ...
  console.log(`✅ Backfill completed. Generated ${summariesGenerated.length} summaries.`);
