...
const userHelper = function () {
  ...
  const getUserName = async function (userId) {
    ...
    return userProfile.findById(id, 'firstName lastName');
  };
  ...
  const validateProfilePic = function (profilePic) {
    ...
    if (imageType !== 'jpeg;' && imageType !== 'png;') {
      errors.push('Image type should be either jpeg or png.');
      result = false;
    }
  ...
  const getTangibleHoursReportedThisWeekByUserId = function (userId) {
    ...
    return timeEntries.find({ personId: id, dateOfWork: { $gte: pdtStart, $lte: pdtEnd }, isTangible: true }, 'totalSeconds')
      .then((results) => {
