...
// gets the descriptions key from the array. Modified to use findIndex to fix array-of-objects lookup.
const getDescriptionKey = (val, warningDescriptions) =>
  warningDescriptions.findIndex((desc) => desc.warningTitle === val);

const sortKeysAlphabetically = (warningDescriptions) => (a, b) =>
  getDescriptionKey(a, warningDescriptions) - getDescriptionKey(b, warningDescriptions);
  ...
  if (Object.keys(issueBlueSquare).length !== 0) {
    size = {
      'Removed Blue Square for No Summary':
        warningsObject['Blu Sq Rmvd - For No Summary']?.length || 0,
      'Removed Blue Square for Hours Close Enough':
        warningsObject['Blu Sq Rmvd - Hrs Close Enoug']?.length || 0,
    };
  }

  const warns = Object.keys(warningsObject)
    .sort(sortKeysAlphabetically(warningDescriptions))
    .reduce((acc, cur) => {
