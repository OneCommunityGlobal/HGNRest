...
const getExpensesByProject = async (req, res) => {
  ...
  try {
    ...
    const totalMaterialsCost = project.materials?.reduce((acc, mat) => {
    ...
    const totalEquipmentCost = project.materials?.reduce((acc, mat) => {
      const rentalDays = mat.purchaseRecord?.reduce((total, record) => {
        if (record.estUsageTime) {
          const days = parseInt(record.estUsageTime.split(' ')[0], 10) || Math.floor(Math.random() * 30) + 1;
          return total + days;
        }
        return total;
    ...
    const responsePayload = {
      ...
      breakdown: [
        ...
        { category: 'Equipment', actualCost: totalEquipmentCost, plannedCost: totalPlannedCost * 0.3 },
        { category: 'Materials', actualCost: totalMaterialsCost, plannedCost: totalPlannedCost * 0.3 }
      ]
    };
