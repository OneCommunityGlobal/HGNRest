    ...
    const processTypes = async () => {
      const processType = async (type) => {
        ...
        try {
          ...
          const processToolItems = async () => {
            const processToolItem = async (toolItem) => {
            ...
            await Promise.all(type.toolItems.map(processToolItem));
          };

          await processToolItems();
      ...
      await Promise.all(typesArray.map(processType));
    };
