const bmDashboardPrototypeController = function (DashboardMetrics, BuildingProject, BuildingMaterial) {
    /**
    ...
    const generateDashboardMetrics = async () => {
      try {
        ...
        const materialCostEstimate = materialStats.length > 0 ? 
          materialStats[0].stockBought * avgCostPerUnit : 27600;
        ...
        const laborCostEstimate = laborStats.length > 0 ? 
          laborStats[0].totalLaborHours * avgHourlyRate : 18400;
          ...
          totalProjects: {
            ...
            trend: {
              value: previousWeekMetrics?.metrics?.totalProjects?.value ? 
                calculateTrend(totalProjects, previousWeekMetrics.metrics.totalProjects.value) : 16,
              period: 'week'
            }
          },
          completedProjects: {
            ...
            trend: {
              value: previousWeekMetrics?.metrics?.completedProjects?.value ? 
                calculateTrend(completedProjects, previousWeekMetrics.metrics.completedProjects.value) : 14,
              period: 'week'
            }
          },
          delayedProjects: {
            ...
            trend: {
              value: previousWeekMetrics?.metrics?.delayedProjects?.value ? 
                calculateTrend(delayedProjects, previousWeekMetrics.metrics.delayedProjects.value) : -18,
              period: 'week'
            }
          },
          activeProjects: {
            ...
            trend: {
              value: previousWeekMetrics?.metrics?.activeProjects?.value ? 
                calculateTrend(activeProjects, previousWeekMetrics.metrics.activeProjects.value) : 3,
              period: 'week'
            }
          },
          avgProjectDuration: {
            ...
            trend: {
              value: previousWeekMetrics?.metrics?.avgProjectDuration?.value ? 
                calculateTrend(avgProjectDuration, previousWeekMetrics.metrics.avgProjectDuration.value) : 13,
              period: 'week'
            }
          },
          totalMaterialCost: {
            ...
            trend: {
              value: previousWeekMetrics?.metrics?.totalMaterialCost?.value ? 
                calculateTrend(materialCostEstimate / 1000, previousWeekMetrics.metrics.totalMaterialCost.value) : 9,
              period: 'week'
            }
          },
          totalLaborCost: {
            ...
            trend: {
              value: previousWeekMetrics?.metrics?.totalLaborCost?.value ? 
                calculateTrend(laborCostEstimate / 1000, previousWeekMetrics.metrics.totalLaborCost.value) : 14,
              period: 'week'
            }
          },
          totalMaterialUsed: {
            ...
            trend: {
              value: previousMonthMetrics?.metrics?.totalMaterialUsed?.value ? 
                calculateTrend(materialData.totalMaterialUsed, previousMonthMetrics.metrics.totalMaterialUsed.value) : 11,
              period: 'month'
            }
          },
          materialWasted: {
            ...
            trend: {
              value: previousMonthMetrics?.metrics?.materialWasted?.value ? 
                calculateTrend(materialData.materialWasted, previousMonthMetrics.metrics.materialWasted.value) : 14,
              period: 'month'
            }
          },
          materialAvailable: {
            ...
            trend: {
              value: previousMonthMetrics?.metrics?.materialAvailable?.value ? 
                calculateTrend(materialData.materialAvailable, previousMonthMetrics.metrics.materialAvailable.value) : -8,
              period: 'month'
            }
          },
          materialUsed: {
            ...
            trend: {
              value: previousMonthMetrics?.metrics?.materialUsed?.value ? 
                calculateTrend(secondaryMaterialUsed, previousMonthMetrics.metrics.materialUsed.value) : 9,
              period: 'month'
            }
          },
          totalLaborHours: {
            value: parseFloat((laborStats.length > 0 ? laborStats[0].totalLaborHours / 1000 : 12.8).toFixed(1)), 
            trend: {
              value: previousMonthMetrics?.metrics?.totalLaborHours?.value ? 
                calculateTrend((laborStats.length > 0 ? laborStats[0].totalLaborHours / 1000 : 12.8), previousMonthMetrics.metrics.totalLaborHours.value) : 17,
              period: 'month'
            }
          }
        };
