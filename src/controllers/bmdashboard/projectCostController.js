...
const controller = function (ProjectCost) {
  ...
  const calculatePredictedCosts = (costs) => {
    ...
    const lastActualCostIndex = costs.findIndex(
      (cost) => cost.actualCost === null || cost.actualCost === undefined,
    );
    const historicalCosts =
      lastActualCostIndex === -1 ? costs : costs.slice(0, lastActualCostIndex);
  ...
  return {
    ...
    deleteProject: async (req, res) => {
      try {
        const { projectId } = req.params;
        const deletedProjectCost = await ProjectCost.findOneAndDelete({
          projectId: Number(projectId),
        });
