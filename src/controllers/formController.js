      ...
      if (formQuestions) {
        ...
        formQuestions.forEach((question) => {
          if (!question.label || typeof question.label !== 'string') {
            return res
              .status(400)
              .json({ message: "Each question must have a valid 'label' of type string" });
          }

          if (!question.type || typeof question.type !== 'string') {
            return res
              .status(400)
              .json({ message: "Each question must have a valid 'type' of type string" });
          }
  ...
  const addDataToForm = async function (req, res) {
    try {
      ...
      if (responses.length !== formQuestions.length) {
        return res
          .status(400)
          .json({ message: 'Number of responses does not match the number of form questions.' });
      }

      const validatedResponses = [];

      responses.forEach((responseItem, i) => {
        ...
        if (!response.questionLabel || response.answer === undefined) {
          return res
            .status(400)
            .json({ message: `Response for question "${question.label}" is incomplete.` });
        }
        ...
        if (question.type === 'radio') {
          ...
          if (response.answer.length === 0) {
            return res
              .status(400)
              .json({ message: `At least one option must be selected for "${question.label}".` });
          }
