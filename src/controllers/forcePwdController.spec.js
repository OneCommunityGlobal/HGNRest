...
describe.skip('ForcePwdController Unit Tests', () => {
  beforeEach(() => {
  ...
  test('Returns a 500 Internal Error status if new password fails to save', async () => {
    const { forcePwd } = makeSut();
    const errorMsg = new Error('Error happened when saving user');
    const mockUser = {
      set: jest.fn(),
