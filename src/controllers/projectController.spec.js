...
describe('projectController module', () => {
  ...
  describe('getAllProjects function', () => {
    test('Ensure getAllProjects returns 404 if there is an error accessing the database', async () => {
      ...
      const dbError = 'Error fetching projects. Please try again.';
      jest.spyOn(Project, 'find').mockImplementation(() => ({
        sort: jest.fn().mockRejectedValue(new Error(dbError)),
      }));
  ...
  describe('postProject function', () => {
    test('Ensure postProject returns 403 if the user does not have postProject permissions', async () => {
      ...
      assertResMock(
        401, //changed from 403 to 401
        'You are not authorized to create new projects.',
        response,
        mockRes,
      );
    });
    ...
    test('Ensure postProject returns 400 if the project name is empty', async () => {
      ...
      const response = await postProject(mockReqModified, mockRes, mockNext);

      assertResMock(400, 'Project Name is mandatory fields.', response, mockRes);
    ...
    test('Ensure postProject returns 400 if any error occurs when finding a project', async () => {
      const { postProject } = makeSut();
      const errorMsg = 'Error creating project. Please try again.';
      const hasPermissionSpy = mockHasPermission(true);
    ...
    test('Ensure postProject returns 400 if the projectName exists', async () => {
      ...
      assertResMock(
        400,
        `Project Name must be unique. Another project with name ${mockReqModified.body.projectName} already exists. Please note that project names are case insensitive.`,
        response,
        mockRes,
      );
    });

    test('Ensure postProject returns 400 any error occurs in saving project', async () => {
      const { postProject } = makeSut();
  ...
  describe('assignProjectToUsers function', () => {
    test('Ensure assignProjectToUsers returns 403 if the user does not have assignProjectToUsers permissions', async () => {
      ...
      assertResMock(403, 'You are not authorized to perform this operation', response, mockRes);
    });
    ...
    test('Ensure assignProjectToUsers returns 400 if the projectId or users is empty', async () => {
      ...
      const response = await assignProjectToUsers(mockReqModified, mockRes);

      assertResMock(400, 'Invalid request', response, mockRes);
    ...
    test('Ensure assignProjectToUsers returns 400 if the project ID provided does not correspond to any project in the database', async () => {
      ...
      assertResMock(400, 'Invalid project', response, mockRes);
    ...
    test('Ensure assignProjectToUsers returns 500 if an error occurs in findById', async () => {
      ...
      const errorMessage = 'Error fetching project. Please try again.';
  ...
  describe('putProject function', () => {
    ...
    test('Ensure putProject returns 403 if the user does not have putProject permissions', async () => {
      ...
      const response = await putProject(mockReqModified, mockRes);
    ...
    test('Ensure putProject returns 400 if an project exist with same projectName', async () => {
      const { putProject } = makeSut();

      const hasPermissionSpy = mockHasPermission(true);

      jest.spyOn(Project, 'find').mockResolvedValue([{ name: 'Sample1', id: '123' }]);

      const response = await putProject(mockReqModified, mockRes);

      assertResMock(400, 'This project name is already taken', response, mockRes);
      expect(hasPermissionSpy).toHaveBeenCalledWith(mockReqModified.body.requestor, 'putProject');
    });
    ...
    test('Ensure putProject returns 400 if an error occurs in `findById`', async () => {
      ...
      jest.spyOn(Project, 'find').mockResolvedValue([]);

      jest.spyOn(Project, 'findById').mockRejectedValue(null);

      const response = await putProject(mockReqModified, mockRes);

      assertResMock(400, 'Error updating project. Please try again.', response, mockRes);
      expect(hasPermissionSpy).toHaveBeenCalledWith(mockReqModified.body.requestor, 'putProject');
    });
    ...
    test('Ensure putProject returns 500 status if an error occurs in saving', async () => {
      ...
      const errorMsg = 'Error updating project. Please try again.';
      ...
      jest.spyOn(Project, 'findById').mockResolvedValue(fakeRecord);

      jest.spyOn(Project.prototype, 'save').mockResolvedValue(fakeRecord);
      ...
      expect(mockRes.status).toHaveBeenCalledWith(400);
      expect(mockRes.send).toHaveBeenCalledWith('Error updating project. Please try again.');
    ...
    test('Ensure putProject returns 200 status and return the record ID on successful save', async () => {
      ...
      assertResMock(200, fakeRecord, response, mockRes);
      expect(hasPermissionSpy).toHaveBeenCalledWith(mockReqModified.body.requestor, 'putProject');
    });
  });
  ...
  describe('getUserProjects function', () => {
    test('Ensure getUserProjects returns a 400 status and the error object when findById fails', async () => {
      ...
      jest.spyOn(userProfile, 'findById').mockResolvedValueOnce(null);
    ...
    test('Ensure getUserProjects returns send a 200 status and the userProject data when findById is successful', async () => {
      ...
      const mockProjects = [mongoose.Types.ObjectId(), mongoose.Types.ObjectId()];
      const mockUser = { projects: mockProjects };
      ...
      jest.spyOn(userProfile, 'findById').mockResolvedValueOnce(mockUser);
      const findProject = jest.spyOn(project, 'find').mockResolvedValueOnce(mockProjectList);

      getUserProjects(mockReqModified, mockRes);
      await flushPromises();

      expect(findProject).toHaveBeenCalledWith(
        { _id: { $in: mockProjects }, isActive: { $ne: false } },
        '_id projectName category',
      );

      expect(mockRes.status).toHaveBeenCalledWith(200);
      expect(mockRes.send).toHaveBeenCalledWith([
        { projectId: mockProjects[0], projectName: 'Project A', category: 'Category A' },
        { projectId: mockProjects[1], projectName: 'Project B', category: 'Category B' },
      ]);
    });
  });
  ...
  describe('getprojectMembership function', () => {
    test('Ensure getprojectMembership returns a 400 status if the projectID is Invalid ', async () => {
      ...
      const response = await getprojectMembership(mockReqModified, mockRes);
      // await flushPromises();
    ...
    test('Ensure getprojectMembership returns 200 and project membership', async () => {
      ...
      const response = await getprojectMembership(mockReq, mockRes);
      // await flushPromises();

      assertResMock(200, mockProjects, response, mockRes);

      expect(mockSort).toHaveBeenCalledWith({ firstName: 1, lastName: 1 });
      expect(hasPermissionSpy).toHaveBeenCalledWith(mockReq.body.requestor, 'getProjectMembers');
    });
