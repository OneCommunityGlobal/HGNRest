...
const mockAuthCheck = (isAuthorized) =>
  jest
    .spyOn(authHelper, 'hasPermission')
    .mockImplementationOnce(() => Promise.resolve(isAuthorized));
...
describe('Roles Management System', () => {
  ...
  describe('Create Operations', () => {
    ...
    it('blocks unauthorized role creation', async () => {
      ...
      assertResponseMock(
        403,
        'You are not authorized to create new roles.',
        response,
        mockResponse,
      );
    });
  ...
  describe('Update Operations', () => {
    it('blocks unauthorized role updates', async () => {
      ...
      assertResponseMock(
        403,
        'You are not authorized to make changes to roles.',
        response,
        mockResponse,
      );
    });
    ...
    it('requires permissions field for updates', async () => {
      ...
      assertResponseMock(
        400,
        { error: 'Permissions is a mandatory field' },
        response,
        mockResponse,
      );
    });
    ...
    it('validates role existence before update', async () => {
      ...
      jest
        .spyOn(RoleModel, 'findById')
        .mockImplementation((roleId, callback) => callback(null, null));
    ...
    it('updates role with valid data', async () => {
      ...
      const updatedRole = {
        ...
        save: jest
          .fn()
          .mockResolvedValue({ roleName: 'SuperAdmin', permissions: ['read', 'write'] }),
      };

      jest
        .spyOn(RoleModel, 'findById')
        .mockImplementation((roleId, callback) => callback(null, updatedRole));
      ...
      assertResponseMock(
        201,
        { roleName: 'SuperAdmin', permissions: ['read', 'write'] },
        response,
        mockResponse,
      );
    });
    ...
    it('handles database errors during update', async () => {
      ...
      jest
        .spyOn(RoleModel, 'findById')
        .mockImplementation((roleId, callback) => callback(null, roleData));
