...
const verifyToken = async (req, res) => {
    try {
        ...
        const response = await axios.get(
            `${TRUTH_SOCIAL_API}/accounts/verify_credentials`,
            {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
...
const getPostHistory = async (req, res) => {
    try {
        const history = await TruthSocialPostHistory.find()
            .sort({ postedAt: -1 })
            .limit(50)
            .lean();
...
const updateScheduledPost = async (req, res) => {
    try {
        ...
        const updatedPost = await TruthSocialScheduledPost.findByIdAndUpdate(
            id,
            updateData,
            { new: true }
        );
