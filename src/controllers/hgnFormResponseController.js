...
const hgnFormController = function () {
  const submitFormResponse = async function (req, res) {
    ...
    if (!userInfo || !general || !frontend || !backend || !followUp || !user_id) {
      return res.status(400).json({ error: 'All fields (userInfo, general, frontend, backend) are required' });
    }
    try {
  ...
  const getRankedResponses = async function (req, res) {
    try {
      ...
      const scoredUsers = responses.map(user => {
        ...
        const averageScore = scoreList.length ? scoreList.reduce((a, b) => a + b, 0) / scoreList.length : 0;
        ...
        const topSkills = allSkills.sort((a, b) => b.score - a.score).slice(0, 4).map(s => s.skill);
