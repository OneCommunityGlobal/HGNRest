...
const educationTaskController = function () {
  ...
  const getTaskSubmissions = async (req, res) => {
    try {
      const { status, studentId, lessonPlanId, course_id } = req.query;
      ...
      // Accept `course_id` as an alias for lessonPlanId when frontend sends course filters.
      const lpFilterId = lessonPlanId || course_id;
      if (lpFilterId) {
