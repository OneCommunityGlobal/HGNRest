...
const logUnansweredFAQ = async function (req, res) {
  try {
    ...
    // normalize the question to reduce dup noise (trim spaces)
    const questionRaw = (req.body.question || '').trim();
    if (!questionRaw) {
      return res.status(400).json({ message: 'Question is required' });
    }

    // OPTIONAL: case-insensitive duplicate check (safer than exact match)
    const escapeRegex = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const existingQuestion = await UnansweredFAQ.findOne({
      question: { $regex: `^${escapeRegex(questionRaw)}$`, $options: 'i' },
    });
    ...
    if (process.env.TEST_MODE === 'true') {
      ...
      if (ownerEmails.length === 0) {
        console.warn('No owner emails found. Proceeding without email notification.');
      }
    }

    const emailMessage = `
      <p>A new unanswered question has been logged:</p>
      <p><strong>Question:</strong> ${questionRaw}</p>
      <p>Please review and add an answer if necessary.</p>
    `;

    // Send email only if we have recipients; never fail the request because of email issues
    if (ownerEmails.length > 0) {
      try {
    ...
    return res.status(201).json({ message: 'Question logged successfully', newUnansweredFAQ });
  } catch (error) {
  ...
  try {
    ...
    const changeHistoryWithNames = await Promise.all(
      faq.changeHistory.map(async (change) => {
        ...
        if (change.updatedBy) {
          const updatedByUser = await UserProfile.findById(change.updatedBy).select(
            'firstName lastName',
          );
          if (updatedByUser) {
