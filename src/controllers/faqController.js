...
const logUnansweredFAQ = async function (req, res) {
  try {
    ...
    if (process.env.TEST_MODE === 'true') {
      ...
      const owners = await UserProfile.find({
        role: { $regex: /^owner$/i },
        isActive: { $ne: false },
      }).select('email');

      ownerEmails = owners.map((o) => (o.email || '').trim()).filter(Boolean);

      if (ownerEmails.length === 0 && process.env.SUPPORT_EMAIL) {
        ownerEmails = [process.env.SUPPORT_EMAIL];
      }
    }
