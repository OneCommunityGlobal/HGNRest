    ...
    try {
      ...
      switch (event_type) {
        ...
        case 'CHECKOUT.ORDER.APPROVED': {
          const paymentStatusWithoutCardC = await postPaymentStatusWithoutCard(
          ...
          if (paymentStatusWithoutCardC.status !== 200) {
            ...
            return res
              .status(500)
              .json({ success: false, error: paymentStatusWithoutCardC?.error });
          }
        ...
        case 'PAYMENT.CAPTURE.COMPLETED': {
          const paymentStatusWithoutCardC = await postPaymentStatusWithoutCard(
            ...
            'A',
            req.body?.resource?.supplementary_data?.related_ids?.authorization_id,
          );
          if (paymentStatusWithoutCardC.status !== 200) {
            if (res.headersSent) return;
            return res
        ...
        default:
          console.log('default webhook type');
      }
