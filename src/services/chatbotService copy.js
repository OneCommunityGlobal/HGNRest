...
// // Use the correct free-tier API and an open model
// const HUGGINGFACE_INFERENCE_URL = process.env.HUGGINGFACE_API_URL || 'https://router.huggingface.co';
...
//       { inputs: input }, // Matches your Python EmbedRequest(BaseModel)
//       { timeout: 20000 }
//     );

//     // Your Python code returns: return embeddings.tolist()[0]
//     // which is a flat array [0.1, 0.2, ...]
//     if (Array.isArray(response.data)) {
//       return response.data;
//     } else {
//       throw new Error('Modal returned non-array data. Check Python return statement.');
//     }
//   } catch (err) {
//     // If this throws, you'll see the REAL error (404, 502, etc.) in your logs
...
//   const host = PINECONE_HOST || `${indexName}.svc.${process.env.PINECONE_ENVIRONMENT || 'gcp-starter'}.pinecone.io`;
...
//   // We intentionally let errors throw here so the orchestrator can catch them!
//   const response = await axios.post(`${HUGGINGFACE_INFERENCE_URL}/models/${model}`, hfInput, {
...
//   const systemPrompt =
//     'You are an administrative assistant chatbot answering procedural queries. Use the following pieces of retrieved context to answer the question. If you do not know the answer, say so. At the end of your response, cite the source_document and include any relevant URLs or Video Links from the context.';
...
//       } catch (err) {
//         // SUCCESSFUL FALLBACK ROUTING: Output the raw Pinecone text!
...
// console.log("Modal URL:", process.env.EMBEDDING_SERVICE_URL);
...
async function getChatbotReply(message) {
  ...
  try {
    ...
    if (validMatches.length === 0) {
      return {
        reply:
          "I couldn't find any relevant information in our documents to answer that. Could you please rephrase?",
        sources: [],
      };
    }
